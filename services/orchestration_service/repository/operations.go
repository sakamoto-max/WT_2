package repository

import (
	"context"
	"errors"
	"fmt"
	"orchestration_service/types"
	"time"
	"wt/pkg/enum"

	"github.com/jackc/pgx/v5"
)

var (
	ErrNoRowsFound    = errors.New("no rows found")
	ErrNoRowsEffected = errors.New("no rows effected")
)

func (d *DB) FetchDataFromAuth() ([]types.Data, error) {

	ctx, cancel := context.WithTimeout(context.Background(), queryExecutionTime)
	defer cancel()

	query := `
		SELECT 
			id, 
			target_service, 
			task, 
			status, 
			payload, 
			created_at, 
			number_of_tries 
		FROM 
			outbox
		WHERE 
			status = @status
		LIMIT 
			5
	`

	trnx, err := d.AuthPg.Begin(ctx)
	if err != nil {
		return nil, fmt.Errorf("error in creating a transaction : %w", err)
	}

	defer trnx.Rollback(ctx)

	rows, err := trnx.Query(ctx, query, pgx.NamedArgs{"status": enum.TaskNotCompleted})
	if err != nil {
		return nil, err
	}

	var Data []types.Data

	var id string
	var TargetService string
	var task string
	var status string
	var payload any
	var createdAt time.Time
	var numberOfTries *int

	var allIds []string

	for rows.Next() {
		err := rows.Scan(&id, &TargetService, &task, &status, &payload, &createdAt, &numberOfTries)
		if err != nil {
			return nil, fmt.Errorf("error scanning rows : %w", err)
		}

		allIds = append(allIds, id)

		data := types.Data{
			Id:            id,
			TargetService: TargetService,
			Task:          task,
			Status:        status,
			Payload:       payload,
			CreatedAt:     createdAt,
			NumberOfTries: numberOfTries,
		}

		Data = append(Data, data)
	}

	query = `
		UPDATE outbox
		SET 
			status = @status
		WHERE 
			id = @id
	`
	for _, id := range allIds {
		_, err := trnx.Exec(ctx, query, pgx.NamedArgs{"id": id, "status": enum.TaskPending})
		if err != nil {
			return nil, fmt.Errorf("error in updating the task status to pending : %w", err)
		}
	}

	if len(Data) == 0 {
		return nil, ErrNoRowsFound
	}

	rows.Close()

	err = trnx.Commit(ctx)
	if err != nil {
		return nil, fmt.Errorf("error in commiting the transaction : %w", err)
	}

	return Data, nil
}

func (d *DB) UpdateNumberOfTries(ctx context.Context, id int) error {

	trnx, err := d.AuthPg.Begin(ctx)
	if err != nil {
		return fmt.Errorf("error creating a transaction : %w", err)
	}

	defer trnx.Rollback(ctx)

	var numberOfTries int

	err = trnx.QueryRow(ctx, `
		SELECT number_of_tries FROM outbox
		WHERE ID = @id	
	`, pgx.NamedArgs{"id": id}).Scan(&numberOfTries)
	if err != nil {
		return fmt.Errorf("error occured while getting the number of tries : %w", err)
	}

	result, err := trnx.Exec(ctx, `
		UPDATE outbox
		SET number_of_tries = @number
		WHERE id = @id	
	`, pgx.NamedArgs{"number": numberOfTries + 1, "id": id})
	if err != nil {
		return fmt.Errorf("error occured while updatint the number of tries : %w", err)
	}

	if result.RowsAffected() == 0 {
		return ErrNoRowsEffected
	}

	err = trnx.Commit(ctx)
	if err != nil {
		return fmt.Errorf("error in commiting the transaction : %w", err)
	}

	return nil
}

func (d *DB) TaskCompletedUpdateForAuth(ctx context.Context, id string) error {
	result, err := d.AuthPg.Exec(ctx, `
		UPDATE outbox
		SET status = @status
		WHERE id = @id	
	`, pgx.NamedArgs{"status": enum.TaskCompleted, "id": id})
	if err != nil {
		return fmt.Errorf("error updating the task : %w", err)
	}

	if result.RowsAffected() == 0 {
		return fmt.Errorf("didnot affect any rows ")
	}

	return nil
}
func (d *DB) TaskPendingUpdateForAuth(ctx context.Context, id string) error {
	query := `
		UPDATE outbox
		SET status = @status
		WHERE id = @id	
	`
	result, err := d.AuthPg.Exec(ctx, query, pgx.NamedArgs{"status": enum.TaskPending, "id": id})
	if err != nil {
		return fmt.Errorf("error updating the task : %w", err)
	}

	if result.RowsAffected() == 0 {
		return fmt.Errorf("didnot affect any rows ")
	}

	return nil
}
func (d *DB) TaskNotCompletedForAuth(ctx context.Context, id string) error {
	query := `
		UPDATE outbox
		SET status = @status
		WHERE id = @id
	`

	_, err := d.AuthPg.Exec(ctx, query, pgx.NamedArgs{"status": enum.TaskNotCompleted, "id": id})
	if err != nil {
		return fmt.Errorf("error in updating the task to not completed for id %v : %v", id, err)
	}

	return nil
}
